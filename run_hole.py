...
def run_hole(file, name, color):
        if 'pfht1' in name:  #point = [51.435, 51.255, 38.52]
            H = HOLE(file, executable = '/home/semccomas/hole2/exe/hole', cvect = [0,0,1], cpoint = [51.8, 51.65, 67.05], sample = 0.01) #52.935 #diff cvect for pfht1 and glut3
        elif 'glut' in name:
        ...
        H.create_vmd_surface(filename = outdir + '/' + name + 'TM1_7_4_10.hole.vmd')

runf = '%s/%s.TM1_7_4_10.pdb' %(indir, protname)  #input too long for HOLE but this is the same as protname.system.noich.protonly.alignedaxis.finalframe.pdb 
